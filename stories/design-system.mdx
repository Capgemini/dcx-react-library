...
<Meta title="DCXLibrary/Design System" />

# Design System

The [dcx-react-library](https://github.com/capgemini/dcx-react-library) supports opt-in styling customizable via [CSS custom properties](https://developer.mozilla.org/en-US/docs/Web/CSS/Using_CSS_custom_properties).
...
In case that the above methods for custom properties customization are not enought you can resort to inspecting the actual DOM.

All components that support the design system will have the token definitions displayed under the applied styles tab.
