...
The autocomplete is a <b>progressive</b> component that will automatically detect if your browser supports javascript or not. By default, it will render as an input when javascript is enabled, a select otherwise. A custom component can be passed in place of select when javascript is disabled using customNonJSComp attribute.
When this attribute is passed the Autcomplete component will NOT display the select by default but will render whatever custom component is passed. 
When you import the autocomplete component without providing any className or style associated will looks as following:
...
<Autocomplete
  ...
  with swipe gestures."
  customNonJSComp={<FormInput name={'searchfruit'} type={'text'} hiddenErrorText={''}/>}
/>
```
...
# Autocomplete with customNonJSComp

When this property is passed the Autcomplete component will NOT display the select by default but
...
```js
<Autocomplete options={['orange', 'papaya']} customNonJSComp={<FormInput name={'searchfruit'} type={'text'} hiddenErrorText={'errorfruit'}/>}/>
```
